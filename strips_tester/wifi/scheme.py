...
class Scheme(object):
    ...
    def activate(self):
        ...
        # sudo /sbin/ifdown
        #self.interface
        subprocess.check_output(['sudo', '/sbin/ifdown', '--force', 'wlan0-Xperia'], stderr=subprocess.STDOUT)
        time.sleep(30)
        ifup_output = subprocess.check_output(['sudo', '/sbin/ifup', '--force'] + self.as_args(), stderr=subprocess.STDOUT)
        time.sleep(10)
