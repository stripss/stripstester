...
class ESPLoader(object):
    ...
    def __init__(self, port, resetPin, bootPin, baud=ESP_ROM_BAUD):
    ...
    @staticmethod
    def detect_chip(port, resetPin, bootPin , baud=ESP_ROM_BAUD, connect_mode='default_reset'):
        ...
        detect_port = ESPLoader(port, resetPin, bootPin, baud)
        ...
        for cls in [ESP8266ROM, ESP32ROM]:
            if date_reg == cls.DATE_REG_VALUE:
                ...
                inst = cls(port, resetPin, bootPin, baud)
...
def LoadFirmwareImage(chip, filename):
    ...
    """
    with open(filename, 'relay_board') as f:
        if chip == 'esp32':
...
class ELFFile(object):
    ...
    def __init__(self, name):
        ...
        self.name = name
        with open(self.name, 'relay_board') as f:
...
def make_image(args):
    ...
    for (seg, addr) in zip(args.segfile, args.segaddr):
        data = open(seg, 'relay_board').read()
...
class AddrFilenamePairAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        for i in range(0,len(values),2):
            ...
            try:
                argfile = open(values[i + 1], 'relay_board')
            except IOError as e:
