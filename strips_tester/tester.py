...
def start_test_device():
    ...
    while True:
        try:
            task_results = run_custom_tasks()
        # except CriticalEventException as cee:
        #     module_logger.critical("CEE: %s", cee)
        #     continue
        except Exception as e:
            config.on_critical_event(str(e))
